...
class _HomeScreenBodyState extends ConsumerState<HomeScreenBody> {
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint(
      "Weather data loaded for $city: ${response?.current.condition.text}",
    );
    return Stack(
      children: [
        if (response != null)
          BackgroundGif(
            gifName: viewModel.getGifName(response.current.condition.code),
          ),
        if (response != null)
          Positioned.fill(
            child: SafeArea(
                  ...
                  child: ListView(
                    children: [
                      SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // Min/Max Temperature
                            Text(
                              "H:${response.forecast.forecastday[0].day.maxtemp_c.round()}°  L:${response.forecast.forecastday[0].day.mintemp_c.round()}°",
                              style: TextStyle(
                                fontSize: 30,
                                color: Colors.white,
                                shadows: [
                      ...
                      HourlyForecastScroller(
                        ...
                        localTime: DateTime.parse(response.location.localtime),
                        sunrise: vm.sunrise!,
                        sunset: vm.sunset!,
                      ),
